...
from pyFlame.pyFlame.systems.convection_old import ConvectionSystem
...
class FlameSolver(SplitSolver):
    ...
    #     if self.config.flame_type == "premixed":
    #         # Initialize with a tanh profile centered at x_mean
    #         x_mean = 0.5 * (x[0] + x[-1])
    #         width = 0.1 * (x[-1] - x[0])
    #         xi = (x - x_mean) / width
    #         smooth_step = 0.5 * (1 + np.tanh(2*xi))
            
    #         # Temperature profile
    #         self.state[self.i_T] = (self.T_fuel + 
    #                             (self.T_products - self.T_fuel) * smooth_step)
            
    #         # Species profiles
    #         self.state[self.i_Y] = (self.Y_fuel[:, np.newaxis] + 
    #                             (self.Y_products - self.Y_fuel)[:, np.newaxis] * 
    #                             smooth_step[np.newaxis, :])
            
    #         # Simple velocity profile
    #         self.state[self.i_U] = np.ones_like(x)
                
    #     elif self.config.flame_type == "diffusion":
    #         # Initialize velocity field based on strain rate
    ...
    #         if hasattr(self.config, 'use_mixing_layer') and self.config.use_mixing_layer:
    #             # Smooth profiles with mixing layer
    #             width = getattr(self.config, 'mixing_width', 0.002)  # default 2mm
    #             phi = 0.5 * (1 + np.tanh((x - x_center)/width))
                
    #             # Temperature profile
    #             T = self.T_fuel + (self.T_oxidizer - self.T_fuel) * phi
                
    #             # Species profiles with mixing layer
    #             Y = np.zeros((self.n_species, n))
    #             for k in range(self.n_species):
    #                 Y[k] = self.Y_fuel[k] + (self.Y_oxidizer[k] - self.Y_fuel[k]) * phi
    #         else:
    #             # Sharp interface (Ember's approach)
    #             if self.config.fuel_left:
    #                 T = np.where(x < x_center, self.T_fuel, self.T_oxidizer)
    #                 Y = np.zeros((self.n_species, n))
    #                 for k in range(self.n_species):
    #                     Y[k] = np.where(x < x_center, 
    #                                 self.Y_fuel[k], 
    #                                 self.Y_oxidizer[k])
    #             else:
    #                 T = np.where(x < x_center, self.T_oxidizer, self.T_fuel)
    #                 Y = np.zeros((self.n_species, n))
    #                 for k in range(self.n_species):
    #                     Y[k] = np.where(x < x_center, 
    #                                 self.Y_oxidizer[k], 
    ...
    #         # Add optional ignition zone
    #         if hasattr(self.config, 'use_ignition_zone') and self.config.use_ignition_zone:
    #             ignition_width = getattr(self.config, 'ignition_width', 0.001)  # default 1mm
    #             T_peak = getattr(self.config, 'ignition_T_peak', 300)  # default +300K
    #             T += T_peak * np.exp(-(x - x_center)**2/(2*ignition_width**2))
            
    #         self.state[self.i_T] = T
    #         self.state[self.i_Y] = Y
            
    #         # Enforce boundary conditions
    #         if self.config.fuel_left:
    #             self.state[self.i_T, 0] = self.T_fuel
    #             self.state[self.i_Y][:, 0] = self.Y_fuel
    #             self.state[self.i_T, -1] = self.T_oxidizer
    #             self.state[self.i_Y][:, -1] = self.Y_oxidizer
    #         else:
    #             self.state[self.i_T, 0] = self.T_oxidizer
    #             self.state[self.i_Y][:, 0] = self.Y_oxidizer
    #             self.state[self.i_T, -1] = self.T_fuel
    #             self.state[self.i_Y][:, -1] = self.Y_fuel
        
    #     # Update properties
    #     self.correct_mass_fractions()
    #     self.update_properties()
        
    #     # Set momentum source term for diffusion flames
    ...
    #     """Setup boundary conditions based on flame type"""
    #     if self.config.flame_type == "premixed":
    #         # Set up reactants composition
    #         self.gas.set_equivalence_ratio(
    #             self.config.phi,
    #             self.config.fuel,
    #             self.config.oxidizer
    #         )
    #         self.Y_fuel = self.gas.Y
    #         self.T_fuel = self.config.T_fuel
            
    #         # Set up products (equilibrium) composition
    #         self.gas.equilibrate('HP')
    #         self.Y_products = self.gas.Y
    #         self.T_products = self.gas.T
            
    #         # Set boundary conditions
    #         self.grid.left_bc = BoundaryCondition.FIXED_VALUE
    #         self.grid.right_bc = BoundaryCondition.ZERO_GRADIENT
    ...
    #         # Store densities for velocity field calculation
    #         if self.config.fuel_left:
    ...
    # def _update_diffusion_coeffs(self):
    #     """Update diffusion system coefficients"""
    #     # Temperature equation
    #     self.diffusion.D[self.i_T] = self.flame_state.k # Thermal conductivity
    #     self.diffusion.B[self.i_T] = 1.0 / (self.flame_state.rho * self.flame_state.cp)
        
    #     # Species equations
    #     self.diffusion.D[self.i_Y] = self.flame_state.D
    #     self.diffusion.B[self.i_Y] = 1.0 / self.flame_state.rho
        
    #     # Velocity - no diffusion
    #     self.diffusion.D[self.i_U] = 0
    #     self.diffusion.B[self.i_U] = 0
    ...
    def finish_step(self) -> bool:
        ...
        # Optional: Add stability check based on maximum changes
        # max_dT = np.max(np.abs(self.delta_conv[self.i_T] + 
        #                     self.delta_diff[self.i_T] + 
        #                     self.delta_prod[self.i_T])) / self.dt
        
        # if max_dT > 1e6:  # K/s - adjust this threshold as needed
        #     print(f"Temperature changing too rapidly: {max_dT:.2e} K/s")
        #     return False
