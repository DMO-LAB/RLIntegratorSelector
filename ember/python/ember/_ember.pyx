...
cdef class ConfigOptions:
    ...
    def apply_options(self):
        ...
        opts.integratorMaxTimestep = CVODE.maximumTimestep

        # RK23 integrator options
        RK23 = self.rk23Tolerances
        opts.rk23RelTol = RK23.relativeTolerance
        opts.rk23AbsTol = RK23.absoluteTolerance
        opts.rk23MinTimestep = RK23.minimumTimestep
        opts.rk23MaxTimestep = RK23.maximumTimestep
        opts.rk23MaxStepsNumber = RK23.maxStepsNumber

        # RK45 options
        RK45 = self.rk45Tolerances
        opts.rk45AbsTol = RK45.absoluteTolerance
        opts.rk45RelTol = RK45.relativeTolerance
        opts.rk45MinStep = RK45.minimumTimestep
        opts.rk45MaxStep = RK45.maximumTimestep

        # CVODE options
        CVODEZeroD = self.cvodeTolerancesZeroD
        opts.cvode_reltol = CVODEZeroD.relativeTolerance
        opts.cvode_abstol_temperature = CVODEZeroD.abstol_temperature
        opts.cvode_abstol_species = CVODEZeroD.abstol_species
        opts.cvode_minStep = CVODEZeroD.minStep
        opts.cvode_maxStep = CVODEZeroD.maxStep
...
cdef class FlameSolver:
    ...
    def set_integrator_types(self, list integrator_types):
        """
        Set the integrator type for each point in the domain.
        
        Args:
            integrator_types: List of strings specifying integrator type ('cvode', 'qss', or 'rk23')
                            for each point
        """
        if len(integrator_types) != self.solver.T.size():
            raise ValueError("Number of integrator types must match number of points")
            
        cdef vector[string] cpp_types
        for typ in integrator_types:
            if typ not in ['cvode', 'qss', 'rk23', 'bdf', 'seulex', 'boostRK']:
                raise ValueError(f"Invalid integrator type: {typ}")
            cpp_types.push_back(stringify(typ))
            
        self.solver.setIntegratorTypes(cpp_types)

    def set_integrator_tolerances(self, j, reltol, abstol):
        self.solver.setIntegratorTolerances(j, reltol, abstol)

    @property
    def phi(self):
        try:
            self.solver.calculateEquivalenceRatio()
            return getArray_Vec(self.solver.phi)
        except Exception as e:
            print(f"Error calculating equivalence ratio: {e}")
            return None

    @property
    def state_T(self):
        """Get the temperature state from all source terms"""
        cdef vector[double] temps = self.solver.getStateT()
        return np.array([temps[i] for i in range(temps.size())])

    @property
    def state_Y(self):
        """Get the mass fraction states from all source terms"""
        cdef vector[vector[double]] Y_states = self.solver.getStateY()
        return np.array([[Y_states[i][k] for k in range(Y_states[i].size())]
                        for i in range(Y_states.size())])
...
cdef class ZeroDCombustion:
    ...
    def __cinit__(self, *args, **kwargs):
        self.solver = new CxxZeroDCombustion()
    ...
    def set_state(self, double T, np.ndarray[np.double_t, ndim=1] Y):
        """Set the current thermodynamic state"""
        cdef vector[double] Y_vec
        for i in range(len(Y)):
            Y_vec.push_back(Y[i])
        self.solver.setState(T, Y_vec)
        
    def integrate_to_time(self, double tf):
        """Integrate the system to the specified time"""
        return self.solver.integrateToTime(tf)
    
    def integrate_one_step(self, double tf):
        """Take one integration step toward the specified time"""
        return self.solver.integrateOneStep(tf)
        
    def set_integrator_type(self, str integrator_type):
        """Change the type of integrator being used"""
        self.solver.setIntegratorType(stringify(integrator_type))
    
    @property
    def T(self):
        """Current temperature"""
        return self.solver.getTemperature()
    
    @property
    def Y(self):
        """Current mass fractions"""
        cdef vector[double] y_vec = self.solver.getMassFractions()
        cdef np.ndarray[np.double_t, ndim=1] result = np.empty(y_vec.size())
    ...
    def reaction_rates(self):
        ...
        cdef vector[double] rates = self.solver.getReactionRates()
        cdef np.ndarray[np.double_t, ndim=1] result = np.empty(rates.size())
