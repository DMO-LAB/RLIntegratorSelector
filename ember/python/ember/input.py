...
def _usingRK(conf):
    return conf.general.chemistryIntegrator == 'rk'
...
class General(Options):
    ...
    #: variable-order BDF).
    chemistryIntegrator = StringOption("qss", ("cvode", "rk", "boostRK"), level=1)
...
class CvodeTolerances(Options):
    ...
    maximumTimestep = FloatOption(1e-5, level=2, filter=_usingCvode)

class RK23Tolerances(Options):
    """ Tolerances for the RK23 chemistry integrator """
    minimumTimestep = FloatOption(1e-6, level=2, filter=_usingRK)
    maximumTimestep = FloatOption(1e-4, level=2, filter=_usingRK)
    
    relativeTolerance = FloatOption(1e-6, level=2, filter=_usingRK)
    absoluteTolerance = FloatOption(1e-8, level=2, filter=_usingRK)
    
    maxStepsNumber = IntegerOption(100000, level=2, filter=_usingRK)
    
    abstol = FloatOption(1e-11, level=2, filter=_usingRK)
    minval = FloatOption(1e-60, level=2, filter=_usingRK)
    
    stabilityCheck = BoolOption(False, level=2, filter=_usingRK)

class RK45Tolerances(Options):
    """ Tolerances for the RK45 chemistry integrator """
    relativeTolerance = FloatOption(1e-6, level=2, filter=_usingRK)
    absoluteTolerance = FloatOption(1e-8, level=2, filter=_usingRK)
    minimumTimestep = FloatOption(1e-18, level=2, filter=_usingRK)
    maximumTimestep = FloatOption(1e-5, level=2, filter=_usingRK)

class CVODETolerancesZeroD(Options):
    """ Tolerances for the CVODE chemistry integrator """
    relativeTolerance = FloatOption(1e-6, level=2, filter=_usingCvode)
    abstol_temperature = FloatOption(1e-8, level=2, filter=_usingCvode)
    abstol_species = FloatOption(1e-13, level=2, filter=_usingCvode)
    minStep = FloatOption(1e-18, level=2, filter=_usingCvode)
    maxStep = FloatOption(1e-5, level=2, filter=_usingCvode)
...
class Config(object):
    ...
    def __init__(self, *args):
        ...
        self.rk23Tolerances = get(RK23Tolerances)
        self.cvodeTolerancesZeroD = get(CVODETolerancesZeroD)
        self.rk45Tolerances = get(RK45Tolerances)
