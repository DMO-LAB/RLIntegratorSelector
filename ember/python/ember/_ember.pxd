...
cdef extern from "readConfig.h":
    cdef cppclass CxxConfigOptions "ConfigOptions":
        ...
        double integratorSpeciesAbsTol, integratorMinTimestep, integratorMaxTimestep

        double rk23RelTol, rk23AbsTol, rk23MinTimestep, rk23MaxTimestep, rk23MaxStepsNumber

        double rk45AbsTol, rk45RelTol, rk45MinStep, rk45MaxStep

        double cvode_reltol, cvode_abstol_temperature, cvode_abstol_species, cvode_minStep, cvode_maxStep
...
cdef extern from "flameSolver.h":
    cdef cppclass CxxFlameSolver "FlameSolver":
        ...
        void resizeAuxiliary() except +
        void setIntegratorTypes(const vector[string]& types) except +
        void calculateEquivalenceRatio() except +
        vector[double] getStateT() except +
        vector[vector[double]] getStateY() except +
        void setIntegratorTolerances(size_t j, double reltol, double abstol) except +
...
cdef extern from "zeroDCombustion.h":
    cdef cppclass CxxZeroDCombustion "ZeroDCombustion":
        void setOptions(CxxConfigOptions&) except +
        void initialize() except +
        void setState(double, const vector[double]&) except +
        int integrateToTime(double) except +
        int integrateOneStep(double) except +
        double getTemperature() const
        vector[double] getMassFractions() const
        void setIntegratorType(const string&) except +
        vector[double] getReactionRates() const
