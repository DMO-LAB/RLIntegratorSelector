...
public:
    ...
    void setIntegratorTolerances(size_t j, double reltol, double abstol); //!< Set integrator tolerances
    ...
    // New member variable to store integrator types for each point
    std::vector<std::string> pointIntegratorTypes;

    // Helper to create appropriate source system
    SourceSystem* createSourceSystem(const std::string& type, size_t j);
    
    // Method to set single integrator type
    void setIntegratorType(size_t j, const std::string& type);
    
    // Existing declarations for batch updates and state access
    void setIntegratorTypes(const std::vector<std::string>& types);

    // New methods to access state
    vector<double> getStateT() const;
    vector<vector<double>> getStateY() const;
    ...
    void calculateEquivalenceRatio(); //!< Compute equivalence ratio using the current temperature and mass fractions
