...
int FlameSolver::finishStep()
{
    ...
    if (debugParameters::debugTimesteps & (nOutput % 100 == 0)) {
    ...
    if (5 < -10 & nRegrid >= options.regridStepInterval) { // replace t < -10 to  t > tRegrid || nRegrid >= options.regridStepInterval
...
SourceSystem* FlameSolver::createSourceSystem(const std::string& type, size_t j) {
    SourceSystem* system = nullptr;
...
void FlameSolver::setIntegratorTolerances(size_t j, double reltol, double abstol) {
    sourceTerms[j].setTolerances(reltol, abstol);
}


void FlameSolver::setIntegratorTypes(const std::vector<std::string>& types) {
    if (types.size() != nPoints) {
        throw DebugException("Number of integrator types must match number of points");
    }

    // Update each point that needs updating
    for (size_t j = 0; j < nPoints; j++) {
        if (pointIntegratorTypes[j] != types[j]) {
            // std::cout << "Setting integrator type to " << types[j] << " at point " << j << std::endl;
            setIntegratorType(j, types[j]);
        }
    }
}

vector<double> FlameSolver::getStateT() const {
    vector<double> temperatures;
    temperatures.reserve(nPoints);
    for (size_t j = 0; j < nPoints; j++) {
        temperatures.push_back(sourceTerms[j].T);
    }
    return temperatures;
}

vector<vector<double>> FlameSolver::getStateY() const {
    vector<vector<double>> massFractions;
    massFractions.reserve(nPoints);
    for (size_t j = 0; j < nPoints; j++) {
        vector<double> Y_point(nSpec);
        for (size_t k = 0; k < nSpec; k++) {
            Y_point[k] = sourceTerms[j].Y[k];
        }
        massFractions.push_back(Y_point);
    }
...
void FlameSolver::resizeAuxiliary()
{
    ...
    if (nPoints > nPointsOld) {
        // Initialize new points with default integrator type
        for (size_t j = pointIntegratorTypes.size(); j < nPoints; j++) {
            pointIntegratorTypes.push_back(options.chemistryIntegrator);
        }
        
        for (size_t j=nPointsOld; j<nPoints; j++) {
            useCVODE.push_back(pointIntegratorTypes[j] == "cvode");

            SourceSystem* system;
            if (pointIntegratorTypes[j] == "cvode") {
                system = new SourceSystemCVODE();
        ...
        // Remove extra points from pointIntegratorTypes
        pointIntegratorTypes.resize(nPoints);
...
void FlameSolver::calculateEquivalenceRatio() {
    // Get a reference to the local gas object
    CanteraGas& gas = gases.local();
    
    // Ensure gas is initialized
    if (!gas.initialized()) {
        std::lock_guard<std::mutex> lock(gasInitMutex);
        if (!gas.initialized()) {
            gas.setOptions(options);
            gas.initialize();
        }
    ...
    // Resize phi vector if needed
    phi.resize(nPoints);
    
    try {
        for (size_t j=0; j<nPoints; j++) {
            // Set the state using mass fractions and temperature
            gas.setStateMass(&Y(0,j), T(j));
            
            // Get and store the equivalence ratio
            double phiValue = gas.getEquivalenceRatio();
            
            // Check for valid value
            if (std::isfinite(phiValue)) {
                phi[j] = phiValue;
            } else {
                phi[j] = -1.0; // or some other sentinel value
                // logFile.write(format("Warning: Invalid equivalence ratio at point %d") % j);
            }
        }
    } catch (const std::exception& e) {
        logFile.write(format("Error calculating equivalence ratio: %s") % e.what());
        throw;
    }
}
